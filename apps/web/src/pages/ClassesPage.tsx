    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.16em] text-ember">Academic structure</p>
            <h2 className="mt-2 font-display text-4xl font-bold text-ink">Classes</h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-ink/60">
              Create class groups, assign lead teachers, and review student rosters from the shared PostgreSQL database.
            </p>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
            <label className="flex h-11 items-center gap-2 rounded-md border border-ocean/10 bg-sky px-3">
              ...
              <input
                ...
                placeholder="Search class, level, section, or teacher"
                value={search}
                onChange={(event) => {
                  setPage(1);
          ...
          {isLoading ? (
            ...
            <ResponsiveTable<ClassDto>
              ...
              columns={[
                ...
                { key: 'teacher', header: 'Teacher', render: (classRecord) => classRecord.teacher ? `${classRecord.teacher.firstName} ${classRecord.teacher.lastName}` : 'Unassigned' },
                { key: 'room', header: 'Room', render: (classRecord) => classRecord.room ?? 'Not set' },
                { key: 'capacity', header: 'Capacity', render: (classRecord) => classRecord.capacity ? `${classRecord._count?.students ?? 0}/${classRecord.capacity}` : 'Not set' },
                { key: 'students', header: 'Students', render: (classRecord) => classRecord._count?.students ?? classRecord.students?.length ?? 0 },
                {
                  key: 'subjects',
                  header: 'Subjects',
                  render: (classRecord) => classRecord.subjects?.length ? classRecord.subjects.map((subject) => subject.name).join(', ') : 'No subject'
                },
                {
                  key: 'roster',
                  header: 'Roster',
                ...
                },
                {
                  key: 'profile',
                  header: 'Profile',
                  render: (classRecord) => <Link className="font-bold text-ocean hover:text-ember" to={`/classes/${classRecord.id}`}>Open</Link>
                }
              ]}
            />
          ) : (
            <EmptyState
              ...
              description="Create classes for this school, or adjust the filters to see existing groups."
            />
          )}
        </section>
        ...
        {pagination ? (
          <div className="mt-5 flex flex-col justify-between gap-3 rounded-lg border border-ocean/10 bg-white px-4 py-3 text-sm shadow-panel sm:flex-row sm:items-center">
            <p className="text-ink/60">
              Page <span className="font-bold text-ink">{pagination.page}</span> of <span className="font-bold text-ink">{pagination.totalPages}</span> · {pagination.total} classes
            </p>
